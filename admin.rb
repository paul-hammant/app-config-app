...
def useradd(p4port = nil, user = nil, email = nil, password = nil)
  ...
  if (Dir.entries working_copy).length == 2
    puts "aardvark_configuration files not under source control, adding them..."
    ...
    puts %x[p4 -p #{p4port} -u #{user} -P #{password} -c #{client_name} submit -d "Initial import of aardvark_configuration.json/html"]
  end
